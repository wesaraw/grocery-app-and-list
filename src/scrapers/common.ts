 ...
 */
export function matchPack(str: string | null | undefined): RegExpMatchArray | null {
  if (!str) return null;
 ...
 */
export function parseUnitPrice(text: string | null | undefined): UnitPriceInfo | null {
  if (!text) return null;
  text = text.trim();
 ...
 */
export function selectOne<T extends Element = Element>(
  root: ParentNode,
  selectors: string[]
): T | null {
...
export interface StoreStrategy {
  /** Optional pack matcher override. */
  matchPack?: (str: string) => RegExpMatchArray | null;
  /** Selector overrides keyed by logical name (e.g. "price"). */
  selectors?: Record<string, string[]>;
  /** Arbitrary additional data for store-specific quirks. */
 ...
 */
export const storeStrategies: Record<string, StoreStrategy> = {};

export function registerStoreStrategy(id: string, strategy: StoreStrategy): void {
  storeStrategies[id] = strategy;
}

export function getStoreStrategy(id: string): StoreStrategy {
