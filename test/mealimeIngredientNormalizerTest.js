...
(function testGarlicCloveBecomesEach() {
  const warnings = [];
  const ingredient = normalizeIngredient("2 cloves garlic", warnings);
  assert.strictEqual(ingredient.quantity, 2);
  assert.strictEqual(ingredient.unit, "each");
  assert.strictEqual(ingredient.containerUnit, "clove");
  assert.strictEqual(ingredient.containerQuantity, 2);
  assert.strictEqual(ingredient.name, "garlic");
  assert.strictEqual(warnings.length, 0);
})();

(function testMediumCloveKeepsDescriptor() {
  const warnings = [];
  const ingredient = normalizeIngredient("1 medium clove garlic", warnings);
  assert.strictEqual(ingredient.quantity, 1);
  assert.strictEqual(ingredient.unit, "each");
  assert.strictEqual(ingredient.containerUnit, "clove");
  assert.strictEqual(ingredient.sizeDescriptor, "medium");
