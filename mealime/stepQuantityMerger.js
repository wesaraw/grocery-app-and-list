...
const OPTIONAL_DESCRIPTOR_TOKENS = new Set(["baby", "fresh", "small", "medium", "large", "extralarge", "extra", "mini"]);
...
function isUnitWord(token) {
  ...
  return Boolean(UNIT_SYNONYMS[normalized]);
}
...
function selectMatch(normalizedToken, rawTokens, tokenIndex, entries) {
...
function gatherCandidateTokens(normalizedToken, rawTokens, tokenIndex, maxLookahead = 2) {
  const candidates = [];
  const seen = new Set();

  function addCandidate(token, index) {
    if (!token) return;
    const rawToken = rawTokens[index];
    if (isUnitWord(rawToken)) {
      return;
    }
    ...
    candidates.push({ normalizedToken: token, tokenIndex: index });
    seen.add(key);
  }

  addCandidate(normalizedToken, tokenIndex);
  for (let offset = 1; offset <= maxLookahead; offset++) {
    const index = tokenIndex + offset;
    const rawToken = rawTokens[index];
    if (!rawToken) {
      break;
    }
...
function findIngredientMatch(normalizedToken, rawTokens, tokenIndex, entries) {
  const candidates = gatherCandidateTokens(normalizedToken, rawTokens, tokenIndex);
  for (const candidate of candidates) {
    const match = selectMatch(candidate.normalizedToken, rawTokens, candidate.tokenIndex, entries);
    if (!match) {
      continue;
    }
    if (OPTIONAL_DESCRIPTOR_TOKENS.has(candidate.normalizedToken)) {
      const nextToken = normalizeToken(rawTokens[candidate.tokenIndex + 1]);
      if (nextToken && !match.tokens.has(nextToken)) {
        continue;
      }
    }
    return match;
