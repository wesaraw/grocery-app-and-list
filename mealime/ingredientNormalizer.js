...
const CONTAINER_TOKEN_SYNONYMS = {
  clove: "clove",
  cloves: "clove",
  block: "block",
  blocks: "block",
...
function extractContainerUnit(tokens) {
  if (!tokens.length) {
    return { containerUnit: null, consumed: 0 };
  }
  const first = cleanToken(tokens[0]);
  const second = tokens[1] ? cleanToken(tokens[1]) : "";
  const combos = [];
  ...
  for (const combo of combos) {
    if (CONTAINER_TOKEN_SYNONYMS[combo.key]) {
      return { containerUnit: CONTAINER_TOKEN_SYNONYMS[combo.key], consumed: combo.consumed };
    }
  }
  return { containerUnit: null, consumed: 0 };
}
...
export function normalizeIngredient(text, warnings = []) {
  ...
  const containerResult = extractContainerUnit(tokensAfterDescriptor);
  const tokensAfterContainer = tokensAfterDescriptor.slice(containerResult.consumed);
  const unitResult = findUnit(tokensAfterContainer);
  const nameTokens = tokensAfterContainer.slice(unitResult.consumed);
  ...
  let containerUnit = containerResult.containerUnit || null;
  let containerQuantity = containerUnit && quantity !== null ? quantity : null;
  let sizeUsedAsMeasurement = false;
  const containerSupportsSize = Boolean(containerUnit && CONTAINER_UNITS.has(containerUnit));
  const hasUsableSizeInfo = Boolean(sizeInfo && typeof sizeInfo.amount === "number" && sizeInfo.unit);
  if (hasUsableSizeInfo && quantity !== null && containerSupportsSize) {
    containerQuantity = containerQuantity ?? quantity;
  ...
  if (!sizeUsedAsMeasurement && containerUnit && !unit) {
    unit = "each";
  }
